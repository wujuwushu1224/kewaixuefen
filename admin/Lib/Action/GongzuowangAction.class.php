...
class GongzuowangAction extends IndexAction {	
	//网站设置
	public function website() {
		$flag=$_POST['flag'];
		if (empty($flag)) {
			$obj=$this->findObj('Website',"1=1");
			$this->assign('obj',$obj);
			$this->display();
		}else {
	...
	public function column(){
		if (empty($_POST)){
			$list = $this->findList("column", "cid = 0", "sort ,id ");
	...
	public function column_xl(){
		if (empty($_POST)){
			$cid = $_GET['cid'];
			$list = $this->findList("column", "cid = $cid", "sort ,id ");
			$this->assign("list",$list);
			
			$this->assign("column_top",$this->findList("column", "cid = 0", "sort ,id "));
