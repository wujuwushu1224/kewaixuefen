...
{   
 ...
 function select()   
 {   
   $return='<select name="pg_Select" onchange="javascript:window.location.href=this.value">';   
 ...
 {   
  ...
  {   
   ...
   case '4':   
	...
	$this->first_page='首页  ';   
    $this->last_page='  尾页';   
    return '共'.$this->totalpage.'页   '.$this->first_page().$this->pre_page().$this->nowbar().$this->next_page().$this->last_page().'&nbsp;'.$this->select();  
    break;   
   case '5':   
 ...
 {   
  if(!empty($url)){   
   ...
   if(empty($_SERVER['QUERY_STRING'])){   
       //不存在QUERY_STRING时   
    $this->url=iconv('gb2312','utf-8//IGNORE',$_SERVER['REQUEST_URI'])."?".$this->page_name."=";   
   }else{   
       //   
    if(stristr($_SERVER['QUERY_STRING'],$this->page_name.'=')){   
        //地址存在页面参数   
     $this->url=str_replace($this->page_name.'='.$this->nowindex,'',iconv('gb2312','utf-8//IGNORE',$_SERVER['REQUEST_URI']));   
