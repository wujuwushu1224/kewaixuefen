   ...
   /**
    ...
    public function upload($savePath ='')
    {
        ...
        foreach($files as $key => &$file)
            ...
            if(!empty($file['name'])) {
                ...
                //保存至上传文件结果数组中
                $this->uploadFiles[] = &$file;   //注意这里是引用

                // 自动检查附件,如果检查出错，继续上传下一个文件
                if ($this->autoCheck && !$this->check($file)) 
                    continue;

                //保存上传文件,如果出错，继续上传下一个文件
                if (!$this->save($file))
                    continue;
    ...
    private function check(&$file) {
        if($file['error']!== 0) {
            //文件上传失败
            //捕获错误代码
            $file['error'] = $this->error($file['error']);
            return false;
        }
        //文件上传成功，进行自定义规则检查
        //检查文件大小
